				...
				<div class="work_area home">
					<div class="task_area">
						...
						<?php if(isset($employee)){
						 ...
						 ?>
						<form action="<?php echo base_url();?>index.php/Administrator/AccountController/AccountEditInforPage/
							<?php echo $emp->Emp_ID; ?>" 
							method="post" class="infor_area">
							<div class="form_area edit_order">
								<div class="row">
									<div class="label">Employee's name</div>
									<div class="input"><input type="text" name="txtName" value="<?php echo $emp->Emp_Name; ?>"></div>
								</div>
								<div class="row">
									<div class="label">Birthday</div>
									<div class="input"><input type="date" name="dtBirthday" value="<?php echo $emp->Emp_Birthday; ?>"></div>
								</div>
								<div class="row">
								...
								<div class="row">
									...
									<div class="input"><input type="text" name="txtPhone" value="<?php echo $emp->Emp_Phone;?>"></div>
								</div>
								<div class="row">
									<div class="label">Address</div>
									<div class="input"><input type="text" name="txtAddress" value="<?php echo $emp->Emp_Address; ?>"></div>
								</div>
										...
										<select name="sltArea" class="BigArea">
											<optgroup label="Select a province or city">
												<?php foreach ($areas as $area) {
													if((int) $emp->Area_ID == (int) $area->Area_ID){
													?>
										...
										<select name="sltSubArea" class="SubArea">
											<optgroup label="Select a province or city">
												<?php foreach ($sub_areas as $sa) {
													if((int) $emp->SA_ID == (int) $area->SA_ID){
													?>
							...
							<div class="form_area">
								...
								<div class="row">
									...
									<div class="label"><?php echo $emp->Acc_Name; ?></div>
								</div>
								<div class="row">
									<div class="label">Password</div>
									<div class="input"><input type="password" name="txtPassword" value="<?php echo $emp->Acc_Password; ?>"></div>
								</div>
								<div class="row">
										...
										<select name="sltRole" class="cboRole">
											<optgroup label="Grant a privilege for this user">
												<?php foreach ($user_role as $role) {
													if((int) $emp->UR_ID == (int) $role->UR_ID){
													?>
												<option value="<?php echo $role->UR_ID; ?>" selected><?php echo $role->UR_Name; ?></option>
												<?php } else{ ?>
